...
```mermaid
flowchart TD
    subgraph A["Path A — the five DeepCS services (phase 10)"]
...
Run goes quiet unexpectedly in phase 10, check there first.
...
- **GKE.** Not in the plan any more: the Kubernetes phase runs locally on `kind`
  (phase 8), which is precisely because of what follows. Left here because it is
  the trap you would walk into if you reached for the managed version.
  Nodes bill per hour whether or not anything runs on them.
