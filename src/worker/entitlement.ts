...
export async function isEntitled(env: Env, uid: string): Promise<boolean> {
  ...
  const row = await statement.bind(uid).first();
...
/**
 * Unlocks a uid for good, on the word of a verified webhook.
 *
 * A row that is still live is left untouched, which is the idempotency: a
 * redelivered event changes nothing. A revoked row is replaced, because a
 * refund leaves the row in place and the next purchase by that uid is a real
 * one that has to unlock again. The event id must differ for that, or
 * redelivering the refunded purchase's own event would undo the refund.
 */
export async function grant(
  ...
  await statement.bind(args.uid, args.orderId, args.eventId, args.purchasedAt).run();
}
...
export async function revoke(env: Env, orderId: string): Promise<void> {
  ...
  await statement.bind(orderId).run();
}
